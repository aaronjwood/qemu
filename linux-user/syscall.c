...
// Same size as defined in binfmts.h
...
/*
 * This modifies how the the system call is constructed and executed.
 * We want to make sure that we keep any forking processes jailed inside of the emulator.
 */
static abi_long arm_execve(char *filename, char *argv[], char *envp[])
{
    char *i_arg = NULL;
    char *i_name = NULL;
    char **new_argp;
    ...
    if (handle_interpreters(buf, &i_arg, &i_name, &offset) != 0) {
        return -ENOEXEC;
    }
    ...
    case TARGET_NR_execve:
        {
            ...
            *q = NULL;
             ...
             * program's problem.
             */
            ret = get_errno(arm_execve(p, argp, envp));
            unlock_user(p, arg1, 0);
